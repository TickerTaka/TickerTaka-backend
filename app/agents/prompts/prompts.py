...
[판정 기준]
- hallucination: 위 [실제 데이터] 어디에도 없는 수치를 완전히 지어낸 경우만 해당
  예) 재무 데이터에 PER이 없는데 "PER 15.3" 제시, 뉴스에도 없는 영업이익 수치 날조
- ok: 재무/가격 데이터에 없더라도 뉴스·공시 검색 결과에 근거가 있으면 반드시 ok
- ok: 시장 전망치·경쟁사 수치·업계 동향 등 뉴스에서 자주 등장하는 정보는 검증 불가로 ok
- ok: 반올림, 요약, 해석 차이, 검증 불가인 경우 모두 ok
